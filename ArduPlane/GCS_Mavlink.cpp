...
{
  //gcs().send_text(MAV_SEVERITY_NOTICE,"command INT Requested");
  switch(packet.command) {

#if OFFBOARD_GUIDED == ENABLED
    case MAV_CMD_GUIDED_CHANGE_SPEED: {
		//gcs().send_text(MAV_SEVERITY_NOTICE,"speed requested");
     ...
     case MAV_CMD_GUIDED_CHANGE_ALTITUDE: {
		//gcs().send_text(MAV_SEVERITY_NOTICE,"Alt Requested");
     ...
     case MAV_CMD_GUIDED_CHANGE_HEADING: {
		//gcs().send_text(MAV_SEVERITY_NOTICE,"Heading Requested");
        // command is only valid in guided mode
        if (plane.control_mode != &plane.mode_guided) {
            return MAV_RESULT_FAILED;
        }
...
MAV_RESULT GCS_MAVLINK_Plane::handle_command_long_packet(const mavlink_command_long_t &packet)
{
    ...
    case MAV_CMD_DO_LAND_START:
		...
		if (plane.quadplane.in_vtolTakeoffLand() || plane.quadplane.in_manual_vtol_mode() || plane.quadplane.in_qland_qrtl() ) {
