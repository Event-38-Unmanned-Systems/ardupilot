...
#define FRAME_CLASS_OLD_IDX 30
    ...
    // @Bitmask: 0:LevelTransition,1:AllowFWTakeoff,2:AllowFWLand,3:Respect takeoff frame types,4:Use a fixed wing approach for VTOL landings,5:Use QRTL instead of QLAND for failsafe when in VTOL modes, 6: use default batt failsafe.
...
const AP_Param::GroupInfo QuadPlane::var_info2[] = {
    ...
    // @Increment: 10
    // @User: Standard
    AP_GROUPINFO("VELZ_MAX_DN", 23, QuadPlane, pilot_velocity_z_max_dn, 100),
...
void QuadPlane::run_z_controller(void)
{
    ...
    if (now - last_pidz_active_ms > 2000) {
        ...
        pos_control->set_max_speed_z(-pilot_velocity_z_max_dn, pilot_velocity_z_max);
...
float QuadPlane::get_pilot_desired_climb_rate_cms(void) const
{
	...
	return throttle_request * (throttle_request > 0.0f ? pilot_velocity_z_max : pilot_velocity_z_max_dn);
